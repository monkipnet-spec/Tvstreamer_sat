...
bool StreamManager::buildRemapPipeline(
    ...
    // 202.34: SRT+remap+UDP-CBR is paced once, upstream, from the PCR timeline
    // generated by mpegtsmux. tsparse derives buffer timestamps from that PCR
    // and clocksync releases the already padded 7x188 transport in real time.
    // StableUdpOutput switches to an unclocked 1:1 packetizer for this exact
    // mode, so there is no second bitrate clock and no reservoir transition.
    ...
    if (srtRemapPrePaddedCbr) {
        std::cerr << "SRT remap CBR mux 202.34: mux_bitrate=" << cfg.targetBitrate
                  << " source_pcr=mpegtsmux"
                  << " null_stuffing=mpegtsmux"
                  << " upstream_wallclock_pacer=pcr-tsparse+clocksync"
                  << " stable_udp_mode=unclocked-1to1-packetizer"
                  << std::endl;
    }
    if (strictTsNetworkOutput) {
...
void StreamManager::monitorBus(const std::string& id) {
    ...
    if (tvs::stream_protocols::inputKind(state->config) ==
        tvs::stream_protocols::InputProtocolKind::Srt) {
        std::cerr << "SRT input watchdog 202.34: startup_wait_ms=15000"
