...
function updateHlsSynchronizationVisibility() {
  ...
  const softPhase = document.getElementById('streamHlsPcrPhasePacing');
  if (providerClock) providerClock.disabled = !isHls;
  if (softPhase) softPhase.disabled = !isHls;
}
  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full" id="streamHlsSynchronizationRow" style="display:${stream.input_mode==='hls'?'':'none'}"><label>HLS синхронизация</label><div class="checkbox-inline"><input id="streamHlsSlowPcrAssist" type="checkbox" ${stream.hls_slow_pcr_assist ? 'checked' : ''} onchange="if(this.checked){const x=document.getElementById('streamHlsPcrPhasePacing');if(x)x.checked=false;}" /><span>Provider PCR clock (ручной режим)</span></div><small>Для каналов вроде TV3: после стабилизации provider PCR становится фиксированным media clock. Транспортный PCR остаётся синтетическим 20 ms.</small><div class="checkbox-inline" style="margin-top:8px"><input id="streamHlsPcrPhasePacing" type="checkbox" ${stream.hls_pcr_phase_pacing ? 'checked' : ''} onchange="if(this.checked){const x=document.getElementById('streamHlsSlowPcrAssist');if(x)x.checked=false;}" /><span>Provider PCR interval pacing (ручной режим)</span></div><small>203.40: для HLS с доказанным фазовым выбросом на границах сегментов. Интервалы между соседними provider PCR рассчитываются заранее из уже буферизованного TS; готовые пакеты идут через обычный token pacing с локальным bitrate. В output-path нет ожидания следующего PCR, нет PLL и catch-up. Внешний UDP остаётся CBR с synthetic PCR 20 ms и NULL stuffing. Не включать вместе с Provider PCR clock.</small></div>
