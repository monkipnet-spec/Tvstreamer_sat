  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full"><label>HLS синхронизация</label><div class="checkbox-inline"><input id="streamHlsSlowPcrAssist" type="checkbox" ${stream.hls_slow_pcr_assist ? 'checked' : ''} /><span>Slow provider PCR (ручной режим)</span></div><small>По умолчанию выключено. Включайте только для HLS-каналов, где измерения показывают, что provider PCR устойчиво медленнее duration-rate на 1–4%. Режим не меняет PCR транспорта: он только замедляет выдачу полезных TS-пакетов до provider PCR и отключает clock-rate коррекцию reservoir PLL на время lock.</small></div>
