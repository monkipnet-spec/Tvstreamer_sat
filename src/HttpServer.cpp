...
function openStreamModal() {
  openStreamForm({
    ...
    interface_address:'', input_interface_address:'', input_mode:'auto', hls_access_key_mode:'none', hls_access_key_name:'Authorization', hls_access_key_value:'', hls_user_agent:'Mozilla/5.0 TVStreammerSAT5', hls_slow_pcr_assist:false, conditional_access_client:'', test_pattern:false, auto_start:false, remap_enabled:false, cbr:true, target_bitrate:2000000, transcode_enabled:false, transcode_resolution:'1920x1080', transcode_video_codec:'h264', transcode_video_encoder:'auto', transcode_video_bitrate:6000000, hls_archive_enabled:false, hls_archive_hours:24, hls_archive_path:'/var/lib/tvstreammersat5/archive', transcode_audio_codec:'aac', transcode_audio_bitrate:192000,
  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full"><label>HLS синхронизация</label><div class="checkbox-inline"><input id="streamHlsSlowPcrAssist" type="checkbox" ${stream.hls_slow_pcr_assist ? 'checked' : ''} /><span>Provider PCR clock (ручной режим)</span></div><small>По умолчанию выключено. При включении после стабилизации provider PCR становится фиксированным media clock этого HLS-канала независимо от rolling EXTINF-rate. Используйте только для измеренных проблемных каналов. Транспортный PCR остаётся синтетическим 20 ms; меняется только темп выдачи полезных TS-пакетов, reservoir PLL на время lock отключён.</small></div>
...
function saveStream(id) {
  ...
  const payload = {
    ...
    hls_slow_pcr_assist: document.getElementById('streamHlsSlowPcrAssist')?.checked === true,
