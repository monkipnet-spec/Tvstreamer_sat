  ...
  const renderStreamForm = () => {
    ...
    openModal(`
      ...
      <div class="form-grid">
        ...
        <div class="form-row full"><label>HLS синхронизация</label><div class="checkbox-inline"><input id="streamHlsSlowPcrAssist" type="checkbox" ${stream.hls_slow_pcr_assist ? 'checked' : ''} onchange="if(this.checked){const x=document.getElementById('streamHlsPcrPhasePacing');if(x)x.checked=false;}" /><span>Provider PCR clock (ручной режим)</span></div><small>Для каналов вроде TV3: после стабилизации provider PCR становится фиксированным media clock. Транспортный PCR остаётся синтетическим 20 ms.</small><div class="checkbox-inline" style="margin-top:8px"><input id="streamHlsPcrPhasePacing" type="checkbox" ${stream.hls_pcr_phase_pacing ? 'checked' : ''} onchange="if(this.checked){const x=document.getElementById('streamHlsSlowPcrAssist');if(x)x.checked=false;}" /><span>Provider PCR soft phase PLL (ручной режим)</span></div><small>203.38: для HLS с доказанным фазовым выбросом на границах сегментов. Пакеты НЕ ожидают PCR-якорь: обычный token pacing сохраняется, а provider PCR лишь плавно корректирует его темп (шаг до 0.3%/100 ms, предел ±5%). Внешний UDP остаётся CBR с synthetic PCR 20 ms и NULL stuffing. Не включать вместе с Provider PCR clock.</small></div>
...
function saveStream(id) {
  ...
  const payload = {
    ...
    hls_pcr_phase_pacing: document.getElementById('streamHlsPcrPhasePacing')?.checked === true,
